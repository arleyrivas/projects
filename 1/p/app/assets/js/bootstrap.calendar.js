...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = 'Calendar',
        ...
        defaults = {
            weekStart: 1,
            msg_days: ["Domingo", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado"],
            msg_months: ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"],
            msg_today: 'Today',
        ...
        template =  ''+
			    ...
			    '<thead>'+
                                '<th class="sel" colspan="2" id="last"><div class="arrow"><i class="glyphicon glyphicon-chevron-left"></i> Anterior</div></th>'+
                                '<th class="sel" colspan="2" id="next"><div class="arrow">Siguiente <i class="glyphicon glyphicon-chevron-right"></i></div></th>'+
                                '<th colspan="1"></th>'+
                                '<th colspan="2"><h3><span class="month"></span>&nbsp;<span class="year"></span></h3></th>'+        
                            '</thead>'+
                            '<tbody class="calendar-header"></tbody>'+
                            '<tfoot class="calendar-body"></tfoot>'+
                        '</table>'+
                    '',
        ...
        for(var i=1; i<=daysInMonth[live_date.getMonth()]; i++){
            $.each(events.event, function(){
                ...
                if( event_date.getDate() === view_date.getDate() && 
                    ...
                    .removeClass("day")
                    .addClass('event')
                    .attr("quotas", JSON.stringify(this.quotas))
    ...
    Plugin.prototype.renderCalendar = function (date) {
        ...
        this.element.find('.year').empty();
        this.element.find('.month').empty();
        this.element.find('.year').append(mon.getFullYear());
        this.element.find('.month').append(this.months[mon.getMonth()]);
        ...
        for(var i=0; i>=last; i++){
            ...
            for(var j=this.weekStart; j<this.days.length + this.weekStart; j++){
                ...
                if(dow === 0){
                    ...
                    _html += '<td class="' +cls+ '" id="'+id+'" year="' + year + '" month="' + month_ + '" day="' + dow + '">'+'<span>' +dow+ '</span><ul></ul></td>';
                }
                ...
                switch(target[0].nodeName.toLowerCase()) {
                    case 'td':
                        if (target.is('.day')){
                            ...
                            var month = parseInt(target.attr('month'), 10)||1;
                            var year = parseInt(target.attr('year'), 10)||1;
                            var quotas = JSON.parse(target.attr('quotas'));
