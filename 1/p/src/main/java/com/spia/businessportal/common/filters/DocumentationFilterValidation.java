 ...
 *
 */
public class DocumentationFilterValidation implements Filter {
	...
	public DocumentationFilterValidation() {
		super();
	}
	...
	public void fillCriteria(Criteria criteria) {
		...
		if (this.getDocTypeId() != null) {
			criteria.createCriteria("type").add(Restrictions.eq("id", this.getDocTypeId()));
			//criteria.add(Restrictions.eq("type", this.getDocTypeId()));
		}

		if (this.getTest() != null && this.getTest().booleanValue()) {
			criteria.setProjection(Projections.projectionList().add(Projections.groupProperty("nbr"))
