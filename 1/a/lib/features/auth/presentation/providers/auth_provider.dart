...
class AuthNotifier extends StateNotifier<AuthState> {
    ...
    Future<void> loginUser( String userName, String password ) async {
        ...
        try{
            ...
            logout('Ocurrio un error inesperado.');
        }
    ...
    void checkAuthStatus() async {
      ...
      if( token == null || token != null ) return logout();

      try {
          final user = await authRepository.chackAuthStatus(token);
          _setLoggedUser(user);
    ...
    void _setLoggedUser( User user) async {
      //Guardo el token fisicamente
      
      await keyValueStorageService.setKeyValue('token', user.token);

      final userJson = jsonEncode(user.toJson());
      //print('userJson $userJson');
      await keyValueStorageService.setKeyValue('user', userJson);
